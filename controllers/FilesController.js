...
const field_checker = require('./utils/field_checker');
const file_operations = require('./utils/file_operations');
const db_operations = require('./utils/db_operations');
const cache_operations = require('./utils/cache_operations');
...
class FilesController {
	...
	async postUpload(req, res) {
		...
		try {
			if (token) {
				userId = await cache_operations.verify_token(token);
				req.on('data', async (data) => {
					try {
						let parsed_data = JSON.parse(data);
						parsed_data.userId = ObjectId(userId);
						new field_checker(parsed_data);
						//perform file operation
						await file_operations.perform_operation(parsed_data);
						//perform db operation
						await db_operations.perform_operation(parsed_data);
					} catch (e) {
						console.log(`The problem comes from the file and db operations or the filed or json parse. Here is the problem ${e.message}`);
						let object = JSON.parse(e.message);
						res.status(object.status).send({ error: object.error });
					}
				});
			} else {
	...
	async get_folders(req, res) {
		...
		try {
			if (token) {
				try {
					await cache_operations.verify_token(token);
					let folders = await db_operations.getPublicFolders();
					res.status(200).send(folders);
				} catch (e) {
					console.log(`An error was caught here the db_get_public_folders function running into some issues. The issue is ${e.message}`);
					let object = JSON.parse(e.message);
					res.status(object.status).send({ error: object.error });
				}
			} else {
				console.log('An error was caught because there is no token');
				throw new Error(messages.ERROR_MESSAGES.UNAUTHORIZED);
			}
		} catch (e) {
			console.log(`The get_folders function has a problem, it detected a problem which is ${e.message}`);
			let object = JSON.parse(e.message);
			res.status(object.status).send({ error: object.error });
		}
