...
class Folder_object {
    ...
    file_type(data) {
        if (data.type === 'folder') {
            ...
            throw new Error("Type is not folder");
        }
    }
    create_folder(data) {
        file.exists(path, (exists) => {
            if (exists) {
                file.writeFile(this.localPath, this.decryptedData, (err) => {
                    if (err) {
                        throw new Error(`Could not write to ${this.localPath}`);
                    }
                })
            } else {
    ...
    create_path(data) {
        file.mkdir(this.path, { recurssive: true }, (err) => {
            ...
            if (err === null) {
                ...
                file.writeFile(`${this.path}/${this.fileName}`, this.decryptedData, (err) => {
                    // file.writeFileSync(`${path}/${fileName}`, decryptedData);
                    console.log('Creating the file and performing a write operation on it');
                    if (err) {
                        throw new Error(`The file could not be created due to ${err}`);
                    }
                })
            }
        })
